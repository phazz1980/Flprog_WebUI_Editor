...
- **Подключение**: `GET /config`, затем опрос `GET /state?fmt=short` по очереди (`deviceHttp.ts`). **Не открывать** параллельно вкладку `http://IP_МК/` — у МК одно соединение. Опрос с таймаутом и порогом ошибок, чтобы краткая занятость не рвала сессию.
...
**Страница на МК** даёт ссылки автоподключения для **:3002** и **:3000** (вьювер часто на 3002, если редактор занял 3000). Вьювер с **GitHub Pages (HTTPS)** к ESP по HTTP не подключается — нужен локальный `npm start`.
