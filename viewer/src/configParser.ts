...
export function parseConfig(raw: unknown): ViewerConfig | null {
  ...
  for (const row of compactWidgets) {
    ...
    const xRaw = Number(row[2]);
    const yRaw = Number(row[3]);
    const x = Number.isFinite(xRaw) ? xRaw : 0;
    const y = Number.isFinite(yRaw) ? yRaw : 0;
    const widthRaw = Number(row[4]);
    const heightRaw = Number(row[5]);
    const width = Number.isFinite(widthRaw) ? widthRaw : 40;
    const height = Number.isFinite(heightRaw) ? heightRaw : 24;
    const color = typeof row[6] === 'string' ? row[6] : '#3b82f6';
    const text = typeof row[7] === 'string' ? row[7] : '';
    const tabIndexRaw = Number(row[8]);
    const tabIndex = Math.max(0, Math.min(Number.isFinite(tabIndexRaw) ? tabIndexRaw : 0, tabIds.length - 1));
