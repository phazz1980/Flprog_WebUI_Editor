...
function resolveTypeClassName(varType) {
  const normalized = String(varType || '').trim();
  const localTypeMapping = {
    'int': 'IntegerDataType',
    'long': 'LongDataType',
  ...
  if (typeof globalObj.getTypeClassName === 'function') {
    return globalObj.getTypeClassName(normalized);
  }
  if (globalObj.TYPE_MAPPING && typeof globalObj.TYPE_MAPPING === 'object') {
    return globalObj.TYPE_MAPPING[normalized] || 'IntegerDataType';
  }
  return localTypeMapping[normalized] || 'IntegerDataType';
}
const _getTypeClassName = resolveTypeClassName;
...
function createSixxDataTypeSimple(varType, typeId) {
  const typeClass = _getTypeClassName(varType);
...
export function createUbiXmlSixx(
  blockName,
  blockDescription,
  variables,
  functions,
  ...
  xml += '\t\t<sixx.object sixx.id="' + nameStrId + '" sixx.name="name" sixx.type="String" sixx.env="Core" >' + blockLabel + '</sixx.object>\n';
  ...
  xml += '\t\t<sixx.object sixx.name="notCanManyUse" sixx.type="' + (notCanManyUse ? 'True' : 'False') + '" sixx.env="Core" />\n';
  xml += '\t</sixx.object>\n</sixx.object>\n';
  return xml;
}
